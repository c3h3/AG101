...
for case_i in range(n):
    problem_args.append(problem_cases[PROBLEM_LENGTH*case_i:PROBLEM_LENGTH*case_i+PROBLEM_LENGTH])

p = Pool(8)
    
solutions = p.map(prob_solver, problem_args)

for case_i in range(n):
    print "Case #{case_i}: {solution}".format(case_i=case_i+1,
                                              solution=solutions[case_i])